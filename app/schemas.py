...
class WineFeatures(BaseModel):
    ...
    chlorides: float = Field(..., description="Chlorides / sodium chloride (g/dm³)", examples=[0.076])
    free_sulfur_dioxide: float = Field(..., description="Free sulfur dioxide (mg/dm³)", examples=[11.0])
    total_sulfur_dioxide: float = Field(..., description="Total sulfur dioxide (mg/dm³)", examples=[34.0])
    ...
    alcohol: float = Field(..., ge=0, le=100, description="Alcohol content (% volume)", examples=[9.4])


# Response schemas
class PredictionResponse(BaseModel):
    quality: int = Field(..., description="Predicted wine quality (3–8)", examples=[6])
    probability: float = Field(..., description="Model confidence for the predicted class (0–1)", examples=[0.82])
